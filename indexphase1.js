document.addEventListener("DOMContentLoaded", () => {
  ...
  function poseOptionDisplay(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      option.textContent = data[i].english_name;
      option.setAttribute("value", data[i].english_name);
  ...
  function initializeDropdowns() {
    ...
    // Add code to initialize other dropdowns if needed
  }

  initializeDropdowns();

  // Rest of your code for handling user interactions...

  // Example: handle the pose selection
  const dropDownButton = document.getElementById("dropdownMenuButton1");
  dropDownButton.addEventListener("change", (e) => {
    const selectedPose = poses.find((pose) => pose.english_name === e.target.value);
    if (selectedPose) {
      // Update your HTML elements with pose information
      document.getElementById("englishNameText").textContent = `English Name: ${selectedPose.english_name}`;
      document.getElementById("sanskritNameText").textContent = `Sanskrit Pose Name: ${selectedPose.sanskrit_name
        }`;
      document.getElementById("movementTypeText").textContent = `Movement Type: ${selectedPose.movement_type}`;
      document.getElementById("sequenceText").textContent = `Sequence: ${selectedPose.sequence}`;
      document.getElementById("poseImage").src = selectedPose.img_url;
    }
  });
});
